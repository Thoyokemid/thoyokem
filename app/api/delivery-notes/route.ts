...
// PATCH performs a status-transition action: cancel | amend
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'cancel') {
      ...
      // Reverse the stock impact — add the delivered qty back to its warehouse.
      ...
      // Re-mark the parent SO as delivered again.
      const soRows = await readSheet('sales_order');
      const soHeaders = (soRows[0] || []).map((h: any) => String(h ?? '').trim());
      const soIdCol = soHeaders.indexOf('so_id');
      const soStatusCol = soHeaders.indexOf('status');
      ...
      await logActivity({
        ...
        before: null,
        after: { dn_id: newDnId, amended_from: dn_id, status: 'Submitted' },
      });
    ...
    await logActivity({ doctype: 'Delivery Note', documentId: dn_id, action: 'Cancelled', changedBy: session!.user.name || '', before: originalObj, after: currentObj });
