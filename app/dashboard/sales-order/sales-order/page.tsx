...
function SalesOrderDetailPageInner() {
    ...
    try {
      ...
      if (res.ok) {
        if (action === 'amend') {
          ...
          router.push(`/dashboard/sales-order/sales-order?id=${encodeURIComponent(data.so_id)}`);
        } else {
          fetchData();
        }
      } else {
  ...
  return (
      ...
      <DetailView
        ...
        actions={
          so && (
            <>
              <Button variant="secondary" disabled={!perms.print} title={perms.print ? undefined : "Anda tidak punya izin Print"} onClick={() => router.push(`/dashboard/sales-order/sales-order/print?id=${encodeURIComponent(id)}&size=a4`)}>
                ...
                fields={[
                  {
                    label: 'Customer',
                    value: (
                      <Link href={`/dashboard/sales-order/customer?id=${encodeURIComponent(so.customer_id)}`} className="text-primary hover:underline">
                        {so.customer_name}
                      </Link>
                    ),
                  },
                  ...
                  { label: 'Owner', value: so.owner || '-' },
                  {
                    ...
                    value: so.amended_from ? (
                      <Link href={`/dashboard/sales-order/sales-order?id=${encodeURIComponent(so.amended_from)}`} className="text-primary hover:underline">
                        {so.amended_from}
                      </Link>
                    ) : '-',
                  },
