...
function SalesInvoiceDetailPageInner() {
    ...
    <>
      <DetailView
        ...
        actions={
          invoice && (
            <>
              <Button variant="secondary" disabled={!perms.print} title={perms.print ? undefined : "Anda tidak punya izin Print"} onClick={() => router.push(`/dashboard/sales-order/sales-invoice/print?id=${encodeURIComponent(id)}&size=a4`)}>
        ...
        {invoice && (
          <div className="space-y-4">
            <DetailSection title="Detail">
              <FieldGrid
                fields={[
                  { label: 'Customer', value: <Link href={`/dashboard/sales-order/customer?id=${encodeURIComponent(invoice.customer_id)}`} className="text-primary hover:underline">{invoice.customer_name}</Link> },
                  { label: 'Sales Order', value: <Link href={`/dashboard/sales-order/sales-order?id=${encodeURIComponent(invoice.so_id)}`} className="text-primary hover:underline">{invoice.so_id}</Link> },
                  { label: 'Delivery Note', value: invoice.dn_id ? <Link href={`/dashboard/delivery-order/delivery-note?id=${encodeURIComponent(invoice.dn_id)}`} className="text-primary hover:underline">{invoice.dn_id}</Link> : '-' },
