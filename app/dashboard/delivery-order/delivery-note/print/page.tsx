  ...
  return (
    <div className={`surat-jalan-wrapper size-${size}`}>
      ...
      <div className="no-print sticky top-0 z-10 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        <button onClick={() => router.push(`/dashboard/delivery-order/delivery-note?id=${encodeURIComponent(dn.dn_id)}`)} className="inline-flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-900">
        ...
        <div className="flex items-center gap-2">
          {(['a4', 'f4', 'thermal'] as PaperSize[]).map((s) => (
            <button
              key={s}
              onClick={() => router.push(`/dashboard/delivery-order/delivery-note/print?id=${encodeURIComponent(dn.dn_id)}&size=${s}`)}
