...
function PurchaseInvoiceDetailPageInner() {
    ...
    <>
      <DetailView
        ...
        actions={
          invoice && (
            <>
              <Button variant="secondary" disabled={!perms.print} title={perms.print ? undefined : "Anda tidak punya izin Print"} onClick={() => router.push(`/dashboard/purchasing/purchase-invoice/print?id=${encodeURIComponent(id)}&size=a4`)}>
        ...
        {invoice && (
          <div className="space-y-4">
            <DetailSection title="Detail">
              <FieldGrid
                fields={[
                  { label: 'Supplier', value: <Link href={`/dashboard/purchasing/supplier?id=${encodeURIComponent(invoice.supplier_id)}`} className="text-primary hover:underline">{invoice.supplier_name}</Link> },
                  { label: 'Purchase Order', value: <Link href={`/dashboard/purchasing/purchase-order?id=${encodeURIComponent(invoice.po_id)}`} className="text-primary hover:underline">{invoice.po_id}</Link> },
...
export default function PurchaseInvoiceDetailPage() {
  return (
    <Suspense fallback={null}>
      <PurchaseInvoiceDetailPageInner />
    </Suspense>
  );
}
