...
function PurchaseOrderDetailPageInner() {
    ...
    try {
      ...
      if (res.ok) {
        if (action === 'amend') {
          ...
          router.push(`/dashboard/purchasing/purchase-order?id=${encodeURIComponent(data.po_id)}`);
        } else {
          fetchData();
        }
      } else {
  ...
  return (
      ...
      <DetailView
        ...
        actions={
          po && (
            <>
              <Button variant="secondary" disabled={!perms.print} title={perms.print ? undefined : "Anda tidak punya izin Print"} onClick={() => router.push(`/dashboard/purchasing/purchase-order/print?id=${encodeURIComponent(id)}&size=a4`)}>
                ...
                fields={[
                  {
                    label: 'Supplier',
                    value: (
                      <Link href={`/dashboard/purchasing/supplier?id=${encodeURIComponent(po.supplier_id)}`} className="text-primary hover:underline">
                        {po.supplier_name}
                      </Link>
                    ),
                  },
                  ...
                  { label: 'Owner', value: po.owner || '-' },
                  {
                    ...
                    value: po.amended_from ? (
                      <Link href={`/dashboard/purchasing/purchase-order?id=${encodeURIComponent(po.amended_from)}`} className="text-primary hover:underline">
                        {po.amended_from}
                      </Link>
                    ) : '-',
                  },
